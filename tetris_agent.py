...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 10 19:38:17 2024
...
class TetrisAgent(object):
    ...
    def getAllPiecePos(self, worldState):
        ...
        self.tempState.stone = worldState.mino_block
        self.tempState.stone_x = worldState.dx
        self.tempState.stone_y = worldState.dy
        self.tempState.currentStoneId = worldState.mino
        self.tempState.currentRotation = worldState.rotation
        self.tempState.blockMobile = worldState.mobile
        self.tempState.nextStoneId = worldState.next_mino
        self.tempState.next_stone = worldState.next_mino_block
        
        # start move
        startMove = self.ExtendedTetrisAction(self.tempState.rotation, self.tempState.dx, self.tempState.dy, self.tempState.mobile, self.tempState.mino_block, [], self.tempState.board)
        liveMoves = []
        ...
        while len(liveMoves) > 0:
            ...
            for actNum in actOrder: # make each possible move
                for liveMove in liveMoves:
                    ...
                    thisMove = self.ExtendedTetrisAction(self.tempState.rotation, self.tempState.dx, self.tempState.dy, self.tempState.mobile, self.tempState.mino_block, thisActList, board)
                    #print(" blockMobile after "+  str(tempState.blockMobile))
                    # if block is active, make extendedTetrisAction and add to liveMoves
                    if self.tempState.mobile:
